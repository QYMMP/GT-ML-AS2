...
def plot_data(title, data, column_prefixes=None, validate_only=False, nn_curve=False, clear_existing=True,
    ...
    if column_prefixes is not None:
        ...
        if not nn_curve:
            ...
            plt.fill_between(data.index, mean - std,
                             mean + std, alpha=0.2)
            plt.plot(data.index, mean, '-', linewidth=1,
...
def plot_mimic_data(problem_name, mimic_files, output_dir, nn_curve=False):
    graph_ys = ['fitness', 'time'] if not nn_curve else []
    ...
    for samples in mimic_files.keys():
        ...
        for keep in mimic_files[samples].keys():
            ...
            for m in mimic_files[samples][keep].keys():
                ...
                dfs = read_data_files(
                    mimic_files[samples][keep][m], nn_curve=nn_curve)
            ...
            if nn_curve:
                ...
                p.savefig(
                    '{}/{}/MIMIC_{}_{}_{}.png'.format(
                        output_dir, problem_name, samples, keep, y.capitalize()),
                    format='png', dpi=150)
            else:
                for y in graph_ys:
                    p = plot_data('{} - MIMIC {} {}: {} vs Iterations'.format(problem_name, samples, keep,
    ...
    for pop in ga_files.keys():
        ...
        for i, mate in enumerate(ga_files[pop].keys()):
            ...
            for mutate in ga_files[pop][mate].keys():
                ...
                dfs = read_data_files(
                    ga_files[pop][mate][mutate], nn_curve=nn_curve)
                step_df = process_step_df(dfs, graph_ys)
                ...
                if nn_curve:
                    ...
                    for df in dfs:
                        df.columns = ['{}_{}'.format(
                            mutate, str(col)) for col in df.columns]
                    main_df[mate].extend(dfs)
                else:
                    for y in graph_ys:
                        df = step_df[y]
                        main_df[y]['{}_mean'.format(
                            mutate)] = np.mean(df, axis=1)
                        main_df[y]['{}_std'.format(
                            mutate)] = np.std(df, axis=1)

            if nn_curve:
                # For the NN problem convergence happens relatively early (except for SA)
                main_df[mate] = reduce(lambda x, y: pd.merge(
                    x, y, on='iterations'), main_df[mate])
                ...
                for y in graph_ys:
                    ...
                    p.savefig(
                        '{}/{}/GA_{}_{}_{}.png'.format(output_dir,
                                                       problem_name, pop, mate, y.capitalize()),
                        format='png', dpi=150)
        if nn_curve:
            p.savefig(
                '{}/{}/GA_{}_{}.png'.format(output_dir,
                                            problem_name, pop, 'Score'),
                format='png', dpi=150)
...
def plot_sa_data(problem_name, sa_files, output_dir, nn_curve=False):
    ...
    for ce in sa_files.keys():
        ...
        if nn_curve:
            dfs = list(dfs.values())
            for df in dfs:
                df.columns = ['{}_{}'.format(ce, str(col))
    ...
    if nn_curve:
        ...
        for y in graph_ys:
            ...
            p.savefig(
                '{}/{}/SA_{}.png'.format(output_dir,
                                         problem_name, y.capitalize()),
                format='png', dpi=150)
...
def plot_backprop_data(problem_name, backprop_files, output_dir, nn_curve=False):
    ...
    if nn_curve:
        ...
        p = plot_data('{} - Backprop: {} vs Iterations'.format(problem_name, y_label), main_df,
                      ...
                      y_label=y_label)
        p.savefig(
            '{}/{}/Backprop_{}.png'.format(output_dir,
...
def plot_best_curves(problem_name, files, output_dir, nn_curve=False):
    ...
    output_file_name_regex = re.compile(
        '{}_([A-Za-z]+)(.*)_LOG\.csv'.format(problem_name))
    prefixes = []
    for algo in files:
        logger.info(files)
        if algo == 'Backprop':
            files[algo] = ['output//NN_OUTPUT\\NN_Backprop_LOG.csv']
        file = files[algo][0]
...
def read_and_plot_test_output(base_dir, output_dir, problem_name, multiple_trials=True, nn_curve=False):
    ...
    if 'MIMIC' in files:
        plot_mimic_data(
            problem_name, files['MIMIC'], output_dir, nn_curve=nn_curve),
    if 'GA' in files:
    ...
    if 'Backprop' in files:
        plot_backprop_data(
            problem_name, files['Backprop'], output_dir, nn_curve=nn_curve)
    if 'RHC' in files:
        plot_rhc_data(problem_name, files['RHC'],
                      output_dir, nn_curve=nn_curve)
        ...
        if best_value > files[algo]['best']:
            if nn_curve:
                ...
                similar_files = glob.glob(
                    '{}/{}_{}{}*_LOG.csv'.format(base_dir, problem_name, algo, params))
                ...
                for i, f in enumerate(list(similar_files)):
                    file_fitness_data = read_data_file(
                        f, nn_curve=nn_curve)[['fitness']]
                    fitness_data = pd.concat(
                        [fitness_data, file_fitness_data], axis=1)
...
if __name__ == '__main__':
    for problem_name in to_process:
        ...
        plot_best_curves(problem_name, best_files, output_path,
                         nn_curve=problem['nn_curve'])

    # TODO: This is wrong, right? Doesn't take into account the averaging of values?
    with open(input_path + '/best_results.csv', 'w+') as f:
        f.write('problem,algorithm,params,best fitness,best iterations,best time\n')
